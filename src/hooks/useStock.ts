...
async function queryStock(
  ...
  const SELECT = `id, device_id, quantity, min_quantity, location, notes, created_at, updated_at,
    device:devices(
  ...
  if (s) {
    ...
    if (isLoteSearch) {
      ...
      if (!loteMov || loteMov.length === 0) return { data: [], count: 0 };
...
export function useStock(search: string) {
  ...
  useEffect(() => {
    ...
    queryStock(search, 0, controller.signal)
      .then(({ data, count }) => {
  ...
  const loadMore = useCallback(() => {
    ...
    queryStock(searchRef.current, offsetRef.current, controller.signal)
      .then(({ data, count }) => {
        if (controller.signal.aborted) return;
      ...
      .catch((e: unknown) => {
        if (controller.signal.aborted) return;
        console.error("loadMore stock error:", e);
      })
  ...
  const refetch = useCallback(() => {
    ...
    setItems([]);
    setLoading(true);
    queryStock(searchRef.current, 0, controller.signal)
      .then(({ data, count }) => {
        setItems(data);
        setTotalCount(count);
        offsetRef.current = data.length;
  ...
  return { items, totalCount, loading, loadingMore, hasMore, loadMore, error, refetch };
}
