...
// FIX: debounce da busca — sem isso, fetchDevices dispara para cada tecla pressionada,
// gerando dezenas de requisições por segundo ao Supabase e travando a UI.
...
const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  if (!file) return;
  // FIX: guard contra duplo clique ou dispatch simultâneo
  if (importing) return;
  ...
  try {
    ...
    if (file.name.endsWith('.csv') || file.name.endsWith('.txt')) {
      ...
      // SEC: JSON.parse + spread operator é vulnerável a prototype pollution.
      // Um JSON malicioso com "__proto__" ou "constructor" pode corromper Object.prototype.
      // Solução: parse, validar que é um objeto plano, e copiar apenas campos conhecidos.
      let parsed: unknown;
      try {
      ...
      if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
        toast.error("JSON deve ser um objeto com campos 'devices' ou 'dispositivos_medicos'.");
        return;
      }
      // Copia apenas os campos conhecidos e seguros — nunca faz spread direto de JSON externo
    ...
    if (res.error) {
      ...
      // FIX: lê o body de erro da Edge Function (mesmo padrão do AdminUsers)
      ...
      setPage(0); fetchDevices(debouncedSearch, 0);
    }
  } catch (err) {
...
// FIX: useEffect depende de debouncedSearch (não de search) para evitar
// uma requisição por tecla pressionada. O debounce de 350ms agrupa keystrokes.
useEffect(() => {
...
const handleSave = async () => {
  ...
  try {
    if (isNew) {
      ...
      // Desestruturamos id do resultado validado para não enviá-lo no update
