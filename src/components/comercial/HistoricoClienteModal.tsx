...
export function HistoricoClienteModal({ clienteId, clientes, onClose }: {
  ...
  const statusLabels: Record<string, string> = {
    pendente: t("historicoClienteModal.status.pendente"), separando: t("historicoClienteModal.status.separando"), pronto: t("historicoClienteModal.status.pronto"),
    faturado: t("historicoClienteModal.status.faturado"), enviado: t("historicoClienteModal.status.enviado"), cancelado: t("historicoClienteModal.status.cancelado"), retorno: t("historicoClienteModal.status.retorno"),
  };
      ...
      <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl bg-card border border-border/30 shadow-xl overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] animate-in fade-in slide-in-from-bottom-4 duration-200">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border/20 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold">{t("historicoClienteModal.title")}</p>
              <p className="text-[11px] text-muted-foreground">{cliente?.nome ?? t("historicoClienteModal.defaultClient")}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          ...
          {!loading && pedidos.length === 0 && (
            <div className="text-center py-10 text-sm text-muted-foreground">{t("historicoClienteModal.noOrders")}</div>
          )}
              ...
              <div key={p.id} className="rounded-xl border border-border/20 bg-background/50 p-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] text-muted-foreground/70">
                    {new Date(p.created_at).toLocaleDateString(t("historicoClienteModal.localeCode"), { day: "2-digit", month: "2-digit", year: "2-digit" })}
                  </span>
                ...
                <div className="space-y-0.5">
                  {p.itens.slice(0, 3).map((i, idx) => (
                    <div key={idx} className="flex items-center justify-between text-[11px]">
                      ...
                      <span className="font-semibold shrink-0 ml-2">{i.quantidade} {t("historicoClienteModal.units")}</span>
                    </div>
                  ))}
                  {p.itens.length > 3 && <p className="text-[10px] text-muted-foreground/50">+{p.itens.length - 3} {t("historicoClienteModal.itemsTotal", { total: totalItens })}</p>}
                </div>
              </div>
            );
          })}
