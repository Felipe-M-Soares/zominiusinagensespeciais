-- Função que retorna os lotes com saldo positivo na fase intermediária.
-- Chamada via supabase.rpc('get_lotes_intermediario') — sem filtros problemáticos no cliente.
...
AS $$
  SELECT
    ...
    upper(sm.lote)         AS lote,
    SUM(
      CASE sm.type
        WHEN 'entrada' THEN sm.quantity
        WHEN 'saida'   THEN -sm.quantity
        ELSE 0
      END
    )::integer             AS saldo,
    d.model,
