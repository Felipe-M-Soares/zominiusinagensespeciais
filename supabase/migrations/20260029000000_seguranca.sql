...
-- SECURITY/INTEGRIDADE (2ª correção): se o pedido já tinha passado pela
-- separação (marcar_pedido_pronto — lotes_separados populado), a quantidade
-- física já tinha sido debitada da expedição. Sem devolver isso aqui,
-- cancelar um pedido depois de separado fazia a peça "sumir" da expedição
-- pra sempre, mesmo a venda nunca tendo se concretizado (ex: pedido volta
-- pro comercial pra correção — RetornarPedidoModal, status 'retorno' — e a
-- vendedora cancela ou remove um item dele).
...
-- remove_pedido_item — remove UMA peça do pedido, devolvendo reserva E, se
-- aplicável, a parte já separada dela na expedição. Substitui o fluxo
-- anterior do frontend (DELETE direto em pedido_itens + RPC
-- release_item_reservation em duas chamadas separadas, sem tocar em
...
-- O fix do token_api (Supabase Vault) foi movido para
-- 20260040000000_correcoes_pendentes.sql — esta migration já tinha sido
-- aplicada no banco antes dessa correção existir.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- admin_clear_history: DROP movido para 20260040000000_correcoes_pendentes.sql
-- (esta migration já tinha sido aplicada no banco antes da remoção existir).
...
-- Agendamento via pg_cron movido para 20260040000000_correcoes_pendentes.sql
-- (esta migration já tinha sido aplicada no banco antes do agendamento existir).
