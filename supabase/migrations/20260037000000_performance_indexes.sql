...
-- ── financeiro_lancamentos: extrato por período e criador ─────────────────────
CREATE INDEX IF NOT EXISTS idx_financeiro_lanc_data_created
  ON public.financeiro_lancamentos (data DESC, created_by)
  WHERE data IS NOT NULL;

-- ── financeiro_lancamentos: tipo + conta (relatórios de DRE) ─────────────────
CREATE INDEX IF NOT EXISTS idx_financeiro_lanc_tipo_conta
  ON public.financeiro_lancamentos (tipo, conta_bancaria_id, data DESC)
...
-- ── stock_items: busca por referência ────────────────────────────────────────
