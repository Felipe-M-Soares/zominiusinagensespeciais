...
-- SECURITY: p_user_id é ignorado de propósito — a função é GRANTed a
-- "authenticated" e pode ser chamada diretamente via RPC pelo cliente
-- (supabase.rpc), não só internamente por outras funções. Se o valor
-- vindo do parâmetro fosse usado, qualquer usuário autenticado poderia
-- passar o UUID de outra pessoa e esgotar o limite dela (bloqueio/DoS
-- contra cancel_pedido, faturar_pedido_sefaz, etc.). Sempre usa o uid
-- da sessão que está de fato chamando, nunca o parâmetro.
...
BEGIN
  ...
  INSERT INTO public.rate_limit_log (user_id, action) VALUES (v_user_id, p_action);
  RETURN true;
END; $f10$;
